...
export class HookService {
  ...
  public async handleHook(body: unknown): Promise<void> {
    ...
    const birthdayField = fields.find(
      (field) => field.field_id === BIRTHDAY_FIELD_ID,
    );
    ...
    const currentAgeField = fields.find(
      (field) => field.field_id === AGE_FIELD_ID,
    );
    const currentAge = currentAgeField
      ? Number(currentAgeField.values[0].value)
      : null;
    ...
    const fieldsToUpdate: AmoCustomField[] = [
      { field_id: AGE_FIELD_ID, values: [{ value: age }] },
    ];
